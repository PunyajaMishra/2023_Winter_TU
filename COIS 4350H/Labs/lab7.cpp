...
http://monismith.info/cs599/examples/quicksortMPI.c

For the lab you only need to muck with the main method and I give you most of it
When you run this mpirun -np X filename  make sure that the number of processors is a power of 2 (2,4, 8, 16, 32, 64)
...
//Main this is the method you'll need to mess with
int main(int argc, char ** argv)
{
	
	
	// boring stuff we'll need
...
// array creation
  ...
  // equivalent to int *arr = new int [N]
  int * arr = (int *) malloc(sizeof(int)*N/size);
  
  int * recvBuffer = (int *) malloc(sizeof(int)*N/size);
  

// Creates the array with random numbers
  int i, j;
  for(i = 0; i < N/size; i++)
    arr[i] = rand()%X;

// this is some stuff for seeing how long the whole thing takes and starting us off.   It picks the mid point element in the array
  if(rank == 0)
  {
    start = MPI_Wtime();
...
// YOU NEED TO WRITE CODE HERE -  YOU NEED TO BROADCAST  (MPI_Bcast (parmeter1, 1, MPI_INT, parameter2, MPI_COMM_WORLD)
