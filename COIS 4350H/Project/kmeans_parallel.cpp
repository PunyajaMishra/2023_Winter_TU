...
int main() {
    ...
    //The Cluster Initialization and the Point initialization are done in parallel by specifying them into sections
#pragma omp parallel
    {
#pragma omp sections
        {
...
// parallelizing the for loop, with only the vector of Points is shared and distance and index is private. Static thread scheduling has been chosen
